...
export type WineDraft = Omit<
  WineRecord,
  "rank" | "rankingUpdatedAt" | "quantity" | "addedAt" | "image"
> & { quantity?: number };
...
export type PluginConfig = { repoPath: string; branch?: string };

type QueryResolution =
  | { status: "matched"; wine: WineRecord }
  | { status: "ambiguous"; options: Array<Pick<WineRecord, "id" | "name" | "vintage">> }
  | { status: "not_found" };
...
export function resolveWineQuery(wines: WineRecord[], query: string): QueryResolution {
  const normalizedQuery = normalize(query);
  const tokens = queryTokens(query);

  if (!normalizedQuery || tokens.length === 0) return { status: "not_found" };
  ...
  if (exact.length === 1) return { status: "matched", wine: exact[0] };

  const candidates = wines.filter((wine) => {
    const haystack = normalize(
      `${wine.id} ${wine.name} ${wine.winery} ${wine.style} ${wine.vintage}`,
    );
    return tokens.every((token) => haystack.includes(token));
  });

  if (candidates.length === 1) return { status: "matched", wine: candidates[0] };
...
export function rankWines(wines: WineRecord[], tieOrder: string[] = []) {
...
export function consumeInventory(wines: WineRecord[], query: string, quantity = 1) {
  if (!Number.isInteger(quantity) || quantity < 1) {
    throw new Error("La cantidad a descontar debe ser un entero positivo.");
  }

  const resolution = resolveWineQuery(wines, query);
  if (resolution.status !== "matched") return { resolution, wines, consumed: null };
  ...
  const remaining = wines.flatMap((wine) => {
    if (wine.id !== target.id) return [wine];
...
export function applyRerank(wines: WineRecord[], entries: RankingEntry[]) {
  ...
  for (const id of currentIds) {
    if (!suppliedIds.has(id)) throw new Error(`Falta el vino ${id} en el ranking.`);
  }
  for (const id of suppliedIds) {
    if (!currentIds.has(id)) throw new Error(`El ranking incluye un vino inexistente: ${id}.`);
  }
...
export function assertBonvivirSource(value: string) {
...
export function assertBonvivirImage(value: string) {
  const url = new URL(value);
  if (
    url.protocol !== "https:" ||
...
function readBoundedBody(response: Response, maxBytes: number, signal?: AbortSignal) {
  if (!response.body) throw new Error("Bonvivir devolvió una respuesta vacía.");
  const length = Number(response.headers.get("content-length") ?? 0);
  if (length > maxBytes) throw new Error("La ficha de Bonvivir supera el límite permitido.");
...
function extractJsonObject(source: string, start: number) {
  if (source[start] !== "{") throw new Error("La ficha embebida de Bonvivir es inválida.");
  let depth = 0;
  let inString = false;
  let escaped = false;
...
function humanizeCellarSlug(value: unknown) {
...
export function extractBonvivirProfileFromHtml(
  html: string,
  sourceUrl: string,
): BonvivirInspection {
  ...
  const data = JSON.parse(extractJsonObject(html, objectStart)) as Record<string, unknown>;
  const wineCard = (data.wineCard ?? {}) as Record<string, unknown>;
  const wineImage = (wineCard.wineImage ?? {}) as Record<string, unknown>;
  const image = optionalString(wineImage.src);
  if (!image) throw new Error("La ficha de Bonvivir no incluye la imagen oficial del vino.");
  ...
  for (const item of rawTechnical) {
    ...
    if (title && description) technical.set(normalize(title), description);
  }
  ...
  const pairingCard = (data.pairingCard ?? {}) as Record<string, unknown>;
  ...
  const field = (name: string) => technical.get(normalize(name)) ?? null;
...
export async function inspectBonvivirWine(sourceUrl: string, signal?: AbortSignal) {
  const source = assertBonvivirSource(sourceUrl);
...
async function downloadImage(value: string, signal?: AbortSignal) {
  ...
  if (!response) throw new Error("No se pudo iniciar la descarga de la imagen.");
  if (!response.ok || !response.body) {
    throw new Error(`No se pudo descargar la imagen de Bonvivir (HTTP ${response.status}).`);
  }

  const length = Number(response.headers.get("content-length") ?? 0);
  if (length > MAX_IMAGE_BYTES) throw new Error("La imagen supera el límite de 5 MB.");
...
function runGit(repoPath: string, args: string[], timeout = 30_000) {
  return new Promise<{ stdout: string; stderr: string }>((resolve, reject) => {
    execFile(
      "git",
      args,
      { cwd: repoPath, encoding: "utf8", timeout, maxBuffer: 1024 * 1024 },
      (error, stdout, stderr) => {
        if (error) {
...
async function openRepository(config: PluginConfig) {
  if (!isAbsolute(config.repoPath)) throw new Error("repoPath debe ser una ruta absoluta.");
  ...
  if ((await realpath(root)) !== repoPath) throw new Error("repoPath no coincide con la raíz Git.");

  const branch = config.branch ?? "main";
  const activeBranch = (await runGit(repoPath, ["rev-parse", "--abbrev-ref", "HEAD"])).stdout.trim();
  if (activeBranch !== branch) throw new Error(`La rama activa debe ser ${branch}.`);

  const remote = (await runGit(repoPath, ["remote", "get-url", "origin"])).stdout.trim();
  if (!ALLOWED_REMOTES.has(remote)) throw new Error("El remoto origin no es MatiasLoiseau/cava-loiseau.");

  return { repoPath, dataPath, imageDirectory, branch };
}

async function readCellar(dataPath: string) {
  const text = await readFile(dataPath, "utf8");
  const wines = JSON.parse(text) as WineRecord[];
  if (!Array.isArray(wines)) throw new Error("data/wines.json no contiene una lista válida.");
...
async function ensureClean(repoPath: string) {
...
async function publishMutation(
  ...
  try {
    ...
    if (newImagePath) await unlink(newImagePath).catch(() => undefined);
    await runGit(repository.repoPath, ["restore", "--staged", "--", ...changedPaths]).catch(
      () => undefined,
    );
    throw error;
  }
...
export async function listCellar(config: PluginConfig) {
...
export async function consumeWine(config: PluginConfig, query: string, quantity = 1) {
  return enqueueMutation(async () => {
    ...
    const publish = await publishMutation(
      repository,
      text,
      ["data/wines.json"],
      `cava: registrar consumo de ${result.consumed!.name}`,
    );
    return {
      changed: true,
...
export async function addWine(config: PluginConfig, draft: WineDraft, signal?: AbortSignal) {
  return enqueueMutation(async () => {
    ...
    const publish = await publishMutation(
      repository,
      text,
      ["data/wines.json", `public/wines/${imageFilename}`],
      `cava: agregar ${record.name} ${record.vintage}`,
      imagePath,
    );
    return {
      changed: true,
...
export async function rerankCellar(config: PluginConfig, entries: RankingEntry[]) {
  return enqueueMutation(async () => {
    ...
    const publish = await publishMutation(
      ...
      text,
      ["data/wines.json"],
      "cava: actualizar ranking de Juan",
    );
    return {
      changed: true,
